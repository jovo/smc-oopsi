function [S_best M_best E_best] = GOOPSI_main_v3_0(F,P,Sim)
...
%
% Outputs (potentially, see last few lines of code)
...
if isfield(Sim,'Out')
    if ~any(Sim.Out=='S'), S_best = []; end
    if ~any(Sim.Out=='M'), M_best = []; end
    if ~any(Sim.Out=='P'), E_best = []; end
end