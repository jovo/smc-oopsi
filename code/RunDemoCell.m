...
switch lab
    case 'takashi'
        ...
        fname=['../../fast-oopsi/data/TS108_6_012.mat']; fprintf('%s\n',fname)
        dt=0.02;                             % time step
    ...
    case 'vincent'
        fname=['../../data/clay_reid/ExampleTrace.mat']; fprintf('%s\n',fname)
        load(fname)
        F=F(3400:4200); F=F-min(F); F=F/max(F); n=0*F;
        fname='stuff'; save(fname,'F','n','dt');
end
clear D R
load(fname);
...
Sim.MaxIter = 2;                   % # of EM iterartions
...
Sim.G_params= 0;                    % estimate observation model {gamma}

% other
Sim.SuppressGraphics=1;             % suppress graphics output
Sim.minVar=[0.1 5];                 % min variance limits for kernel dens est (for mcmc)
Sim.n       = n;                    % true spike times (or simply a place holder, when true spike times are unavailable)     

%% 2) initialize parameters

% initialize particle filter parameters
Nsp         = 3;                    % # of spikes guess (by eye)
P.k         = log(-log(1-Nsp/Sim.T)/Sim.dt); % linear filter (spike log-rate)
P.A         = 150;                  % jump size ($\mu$M)
...
P.tau_c     = 1.0;                  % calcium decay time constant (sec)
P.sigma_c   = P.A/3;                % standard deviation of noise (\mu M)
P.a         = Sim.dt/P.tau_c;
