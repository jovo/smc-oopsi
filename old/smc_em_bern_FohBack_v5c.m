function S = smc_em_bern_FohBack_v5c(Sim,R,B)
...
% 
% this version has 10x var of P[O_s | C_k]
% 
% inputs: 
...
for k=Sim.frac+1:Sim.K-Sim.frac
    ...
    if mod(k*Sim.dt,Sim.dtSample)==0
        S = smc_em_bern_stratresamp_v4(Sim,S,k,U_resamp);   %stratified resample
...
for tt=s:-1:2
    ...
    O.sig2_o(:,tt-1)  = 10*B.a^(-2)*(O.sig2_c(:,tt)+O.sig2_o(:,tt));   %var of P[O_s | C_k]
end
