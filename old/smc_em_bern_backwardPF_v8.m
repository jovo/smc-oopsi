function w_b = smc_em_bern_backwardPF_v8(Sim,S,B)
...
for t=Sim.T-Sim.freq-1:-1:Sim.freq+1
    ...
    w_b(:,t-1)= sum(PHH,1);
%     w_b(:,t-1)= w_b(:,t-1)/sum(w_b(:,t-1));

    
    if any(isnan(w_b(:,t-1))), 
        ass=4; 
    end

    if all(~isnan(w_b(:,t-1))), 
        ass=4; 
    end

    if sum(w_b(:,t-1))<1-1e-5
        ass=4; 
    end
