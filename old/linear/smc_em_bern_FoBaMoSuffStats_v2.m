function [S M] = smc_em_bern_FoBaMoSuffStats_v2(Sim,R,P)
...
for t=Sim.T-Sim.freq-1:-1:Sim.freq
    ...
    mumat       = mu(:,oney)';                          %faster than repmat                     (extra code: C0a = repmat(S.C(:,t-1)',Sim.N,1); n0 = repmat(n1',Sim.N,1); C0b  = B.a*C0a+B.beta*n0;%mumat2 = (B.a*C0mat + B.beta*n1mat);
    ...
    for m=1:Sim.M                                       %for each h term
        ...
        h1      = h1(:,oney);                           %                                       (extra code: h1b = repmat(S.h(:,t,m),1,Sim.N);
        h0      = B.g(m)*S.h(:,t-1,m)+S.n(:,t-1);       %
        h0      = h0(:,oney)';                          %                                       (extra code: h0b = repmat(B.g(m)*S.h(:,t-1,m)'+S.n(:,t-1)',Sim.N,1);
    ...
    T       = T0.*repmat(1./Tn(:)', Sim.N, 1);          %such that each column sums to 1        (extra code: Ta = T0./Tn(oney,:);%Tb= scale_cols(T0,1./Tn);%Td= T0.*(1./Tn(oney,:));

    % compute P[H_t^i, H_{t-1}^j | O]
    PHHn    = (T*S.w_f(:,t-1))';                        %denominator
    PHHn    = PHHn+eps;
    PHHn2   = PHHn(oney,:)';                            %faster than repmat                     (extra code: PHHn2b = repmat(PHHn',1,Sim.N);
