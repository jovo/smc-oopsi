function S = smc_em_bern_BackSampl_v2(Sim,R,B)
...
O.mu_n      = zeros(Sim.N,Sim.freq);            %extize mean spike indicator
O.sig2_n    = zeros(Sim.N,Sim.freq);            %extize variance spike indicator
...
for t=Sim.freq+1:Sim.T-Sim.freq
    ...
    y_k             = B.kx(t)+B.omega'*h';      %input to neuron
    S.p(:,t)        = 1-exp(-exp(y_k)*Sim.dt);  %update p
    ...
    m2  = repmat(O.mu_o(:,t-ss),1,2);            %mean of P[O_s | C_k] for n_k=1 and n_k=0
    v2  = repmat(O.sig2_o(:,t-ss),1,2);          %var of P[O_s | C_k] for n_k=1 and n_k=0
    ...
    log_n           = ones(Sim.N,1);            %extize log sampling spikes
    ...
    %% at observations
    if mod(t,Sim.freq)==0
%         figure(1), plot(R.C(1:t),'k','linewidth',2), hold on, plot(sum(S.w_f(:,1:t).*S.C(:,1:t)),'k'), plot(t,R.O(t),'ok')
...
hhat(:,1,:)     = S.h(:,t,:);                   %initialize hhat
phat(:,1)       = S.p(:,t);                     %initialize phat
for tt=1:s
    % update hhat
    for m=1:Sim.M                               %for each spike history term
    ...
    h(:,1:Sim.M)    = hs(:,1,1:Sim.M);          %this too
    y_t             = B.kx(tt+t)+B.omega'*h';   %input to neuron
