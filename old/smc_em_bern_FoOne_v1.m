function S = smc_em_bern_FoOne_v1(Sim,R,B)
...
for k=2:Sim.K
    ...
    %%%%%%%% sample C
    if mod(k*Sim.dt,Sim.dtSample)==0  %at observation times
        s2=(1/B.sig2_o+1/B.sig2_c)^(-1);
        m=s2*((B.a*S.C(:,k-1)+B.beta*S.I(:,k))/B.sig2_c + R.O(k)/B.sig2_o);
        S.C(:,k)=normrnd(m,s2);
    else                                %at non-observation times
    ...
    %%%%%%%% stratified resample
    if Sim.M>0 && mod(k*Sim.dt,Sim.dtSample)==0
        S = smc_em_bern_stratresamp_v1(Sim,S,R,k,B.sig2_o,U_resamp,Nresamp);
    end %resample
end %for k=2:Sim.K
