function S = smc_em_bern_PriorSampl_v3(Sim,R,B)
...
for t=2:Sim.T
    ...
    if mod(t,Sim.freq)==0
        if B.sig2_o==0
            ...
            C   = P.alpha*S.C(:,t) + P.beta;
            F_mu = C.^P.n./(C.^P.n+P.k_d);
            F_var = sqrt(P.gamma*F_mu + P.zeta).*randn(1,Sim.T);
            R.F = F_mu+F_var;                                  %add noise to observations

            EF          = P.alpha.*(S.C(:,t).^P.n)./(S.C(:,t).^P.n+P.k_D)+P.beta;
            ln_w        = -0.5*(R.F(t)-S.C(:,t)).^2/B.sig2_o;     %compute log of weights
        ...
        S = smc_em_bern_stratresamp_v8(Sim,S,t,U_resamp);
