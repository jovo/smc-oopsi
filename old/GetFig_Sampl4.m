function GetFig_Sampl4(Sim,R,S,M)
...
for i=1:nsubs
    if i==1, 
        subplot(nsubs,1,i), 
        cla, hold on
        tit=['prior mse on spikes=' num2str(sum(M(i).mse.n))];
        title(tit,'fontsize',tfs), 
    elseif i==2, 
        subplot(nsubs,1,i),
        cla, hold on
        tit=['backward mse on spikes=' num2str(sum(M(i).mse.n))];
        title(tit,'fontsize',tfs), 
    elseif i==3, 
        subplot(nsubs,1,i),
        cla, hold on
        tit=['mixture mse on spikes=' num2str(sum(M(i).mse.n))];
        title(tit,'fontsize',tfs), 
    end
    ...
    plot(Sim.tvec,(M(i).Cbar-cmin)/(cmax-cmin)+1,'-.k','linewidth',2)
    stem(Sim.tvec,M(i).nbar,'k','linewidth',1','marker','none')
