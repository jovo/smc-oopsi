...
% E(1:Sim.Ntrials)=EI;
figure(2), clf, hold on, stem([P.tau_c; P.A; P.C_0],'r','linewidth',2), 
...
for k=1:Sim.Ntrials
    ...
    Enew        = GetInitParams_v1(P);
    E(1)        = Enew;
    ErrNew      = norm([Enew.tau_c; Enew.A; Enew.C_0]-[P.tau_c; P.A; P.C_0]);
    ErrOld      = ErrNew;
    dtheta      = inf;
    i           = 0;                    %iteration number
    Sim.conv    = false;
    figure(1), clf, hold on, plot([P.tau_c; P.A; P.C_0],'r'), plot([Enew.tau_c; Enew.A; Enew.C_0],'b'), legend('real')
    while Sim.conv==false;
        ...
        ErrOld  = ErrNew;
        ErrNew  = norm([Enew.tau_c; Enew.A; Enew.C_0]-[P.tau_c; P.A; P.C_0]);
        dtheta  = norm([Enew.tau_c; Enew.A; Enew.C_0]-[Eold.tau_c; Eold.A; Eold.C_0])/norm([Eold.k; Eold.omega]);
        fprintf('\nIteration #%d, Norm of error=%g, derr=%g, dtheta=%g\n',i,ErrNew,ErrOld-ErrNew,dtheta)
        save('M_stuff','Sim','R','P','M','E')
        figure(1), plot([Enew.tau_c; Enew.A; Enew.C_0],'b'), drawnow
        if i>Sim.MaxIter %|| dtheta<.1
            ...
            figure(2), plot([E(k).tau_c; E(k).A; E(k).C_0],'b','linewidth',2), drawnow
        end
