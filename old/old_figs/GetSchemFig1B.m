function GetSchemFig1B(Sim,R,S,B,M)
...
cmin    = min(min(min(R.C(xind)),min(min(S.C(:,xind)))),min(O(xind)));
cmax    = max(max(max(R.C(xind)),max(max(S.C(:,xind)))),max(O(xind)));
...
%% inferred calcium
i=i+1; subplot(Nsubs,1,i), cla,
plot(Sim.tvec,(S.C'-cmin)/(cmax-cmin),'b','LineWidth',1); 
hold on
