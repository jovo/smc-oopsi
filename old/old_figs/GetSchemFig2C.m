function GetSchemFig2C(Sim,R,B,M1,M2)
...
plot(Sim.tvec,(M2.Cbar-cmin)/(cmax-cmin),'linewidth',1,'color',col(2,:))

h=fill([Sim.tvec Sim.tvec(ind)],([M1.Cbar-M1.Cvar M1.Cbar(ind)+M1.Cvar(ind)]-cmin)/(cmax-cmin),ccol(1,:));
set(h,'edgecolor',ccol(1,:))
plot(Sim.tvec,(M1.Cbar-cmin)/(cmax-cmin),'linewidth',1,'color',col(1,:))
...
plot(Sim.tvec,-1*R.I+1.95,'+','Color','k')
