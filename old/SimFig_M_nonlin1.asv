...
E(1)        = P;                %initialize parameter estimates
E(1).k      = 0*E(1).k;
E(1).omega  = 0;
...
E(1).A      = 2*E(1).A;
E(1).C_0    = 2*E(1).C_0;
E(1).sigma_c= 2*E(1).sigma_c;

figure(8), clf, hold on, plot([P.k; P.omega],'r'), plot([E(1).k; E(1).omega],'b'), legend('real')
%%
while Sim.conv==false;
    ...
    plot([E(Sim.i).k; E(Sim.i).omega],'b'), drawnow
    if Sim.i>=Sim.MaxIter || norm(E(Sim.i).k-P.k)<norm(P.k-1.1*P.k)          %check for convergence
        Sim.conv=true;
    end
end
