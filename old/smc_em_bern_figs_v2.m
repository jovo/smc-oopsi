function smc_em_bern_figs_v2(Sim,P,R,S,E,M)
...
spikemax= max(1,max(R.I(xmin:xmax)));
...
stem(Sim.tvec,M.Ibar,'k','Marker','none','LineWidth',2), hold on
plot(Sim.tvec,M.Ibar+sqrt(M.Ivar),'k','LineWidth',1);
hold on
stem(Sim.tvec,R.I,'r','Marker','none','LineWidth',1)%'Color',[0.75 0.75 0.75],)
...
stem(Sim.tvec(1:k),S.I(:,1:k)')
stem(Sim.tvec(1:k),R.I(1:k),'fill')
...
% %%%%%%%% plot E[n] vs. n
% figure, clf, hold on
% ntiles=zeros(max(R.I),5);
% clear lens
% for n=0:max(R.I)
%     nums=find(R.I==n);
%     ntiles(n+1,:) = quantile(M.Ibar(nums),[.025 .25 .50 .75 .975]);
