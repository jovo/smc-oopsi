...
if Sim.Rparams == true
    ...
    if Sim.SpikHis == true
        ...
        [bko Enew.lik_r]  = fminunc(@f_bko,RateParams,options);%find MLE
        ...
        options         = optimset('Display','off','GradObj','off'); %use gradient
        [bk Enew.lik_r]   = fminunc(@f_bk,RateParams,options);%find MLE
...
if Sim.Cparams == true
    ...
    [ve_x fval] = quadprog(M.Q, M.L,[],[],[],[],[0 0 0],[inf inf inf],[1/E.tau_c E.A E.C_0/E.tau_c]);
    Enew.tau_c  = 1/ve_x(1);
    Enew.A      = ve_x(2);
