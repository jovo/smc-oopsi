function S = smc_em_bern_BackSampl_v1(Sim,R,B)
...
S.n         = zeros(Sim.N,Sim.K);               %extize spike counts
...
for k=Sim.freq+1:Sim.K-Sim.freq
    ...
    for m=1:Sim.M                               %for each spike history term
        S.h(:,k,m)=B.g(m)*S.h(:,k-1,m)+S.n(:,k-1)+epsilon_h(:,k,m);
    end
    ...
    % sample I
    S.n(:,k)= U_sampl(:,k)<q_n(:,1);            %sample I
    sp      = S.n(:,k)==1;                      %store index of which samples spiked
    nosp    = S.n(:,k)==0;                      %and which did not
    ...
    if B.sig2_o==0                              %if no observation noise
        ...
        m_c     = v_c.*(O.mu_o(:,k-t)./O.sig2_o(:,k-t)+(B.a*S.C(:,k-1)+B.beta*S.n(:,k))/B.sig2_c);%mean of dist'n for sampling C
    ...
    log_C_Cn        = -0.5*(S.C(:,k)-(B.a*S.C(:,k-1)+B.beta*S.n(:,k))).^2/B.sig2_c;%log P[C_k | C_{k-1}, I_k]
...
%         figure(1), plot(R.C(1:k),'k','linewidth',2), hold on, plot(sum(S.w_f(:,1:k).*S.C(:,1:k)),'k'), plot(k,R.O(k),'ok')
        S = smc_em_bern_stratresamp_v6(Sim,S,k,U_resamp);   %stratified resample
